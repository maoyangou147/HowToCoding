...
class CriteoDataset(Dataset):
    ...
    def __init__(self, root, train=True):
        ...
        if self.train:
            ...
            data = pd.read_csv('/home/bob/code/git_repository/practice-master/Deep_FM/data/train.txt')
            self.train_data = data.iloc[:, :-1].values
            self.target = data.iloc[:, -1].values
        else:
            # data = pd.read_csv(os.path.join(root, 'test.txt'))
            data = pd.read_csv('/home/bob/code/git_repository/practice-master/Deep_FM/data/test.txt')
